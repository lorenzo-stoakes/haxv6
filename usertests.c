...
bigwrite(void)
{
	...
	fprintf(1, "bigwrite test\n");

	unlink("bigwrite");
	for (sz = 499; sz < 12*512; sz += 471) {
		fd = open("bigwrite", O_CREATE | O_RDWR);
		if (fd < 0) {
			fprintf(1, "cannot create bigwrite\n");
			exit();
		}
		int i;
...
fourteen(void)
{
	...
	if (mkdir("12345678901234") != 0) {
		fprintf(1, "mkdir 12345678901234 failed\n");
		exit();
	}
	if (mkdir("12345678901234/123456789012345") != 0) {
		fprintf(1, "mkdir 12345678901234/123456789012345 failed\n");
		exit();
	}
	fd = open("123456789012345/123456789012345/123456789012345", O_CREATE);
	if (fd < 0) {
		fprintf(1, "create 123456789012345/123456789012345/123456789012345 failed\n");
		exit();
	}
	close(fd);
	fd = open("12345678901234/12345678901234/12345678901234", 0);
	if (fd < 0) {
		fprintf(1, "open 12345678901234/12345678901234/12345678901234 failed\n");
		exit();
	}
	close(fd);

	if (mkdir("12345678901234/12345678901234") == 0) {
		fprintf(1, "mkdir 12345678901234/12345678901234 succeeded!\n");
		exit();
	}
	if (mkdir("123456789012345/12345678901234") == 0) {
		fprintf(1, "mkdir 12345678901234/123456789012345 succeeded!\n");
		exit();
	}

	fprintf(1, "fourteen ok\n");
}
...
forktest(void)
{
	...
	if (n == 1000) {
		fprintf(1, "fork claimed to work 1000 times!\n");
		exit();
	}
...
fsfull()
{
	...
	fprintf(1, "fsfull test finished\n");
}
